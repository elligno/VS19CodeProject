...
namespace vsc19 
{
      ...
      m_Q{ nullptr }
    {
      ...
      m_A.reset( new scalarField1D{ std::make_shared<gridLattice1D>(std::get<0>(aGridprms), std::get<1>(aGridprms), 
        std::get<2>(aGridprms)), std::string{ "A-Values" } });
      
        m_Q.reset( new scalarField1D{ std::make_shared<gridLattice1D>(std::get<0>(aGridprms), std::get<1>(aGridprms), 
        std::get<2>(aGridprms)), std::string{ "Q-Values" } });
    }
    bool StateVectorField::reset( const gridLattice1DPtr& aGrid2Reset)
    {
      ...
      m_A.reset( new scalarField1D{ aGrid2Reset, std::string{ "A-Values" } });
      m_Q.reset( new scalarField1D{ aGrid2Reset, std::string{ "Q-Values" } });
